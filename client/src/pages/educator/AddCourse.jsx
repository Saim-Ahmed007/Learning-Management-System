        ...
        <div className='h-screen overflow-scroll flex flex-col items-start justify-between md:p-8 md:pb-0 p-4 pt-8 pb-0'>
            <form onSubmit={handleSubmit} className='flex flex-col gap-4 max-w-md w-full text-gray-500'>
                <div className='flex flex-col gap-1'>
                    <p>Course Title</p>
                    <input type="text" onChange={(e)=>setCourseTitle(e.target.value)} value={courseTitle} className='outline-none md:py-2.5 py-2 px-3 rounded border border-gray-500' placeholder='Type here' required />
                </div>
                <div className='flex flex-col gap-1'>
                ...
                <div className='flex items-center justify-between flex-wrap'>
                    <div className='flex flex-col gap-1'>
                        <p>Course Price</p>
                        <input type='number' onChange={(e)=>setCoursePrice(e.target.value)} value={coursePrice} className='outline-none md:py-2.5 py-2 px-3 w-28 rounded border border-gray-500' placeholder='0' required />
                    </div>
                ...
                <div className='flex flex-col gap-1'>
                    <p>Discount %</p>
                    <input type="number" placeholder='0' required onChange={(e) => setDiscount(e.target.value)} value={discount} min={0} max={100} className='outline-none 
                    md:py-2.5 py-2 px-3 w-28 rounded border border-gray-500 ' />
                </div>
                        ...
                        <div key={chapterIndex} className='bg-white border rounded-lg mb-4'>
                            <div className='flex justify-between items-center p-4 border-b'>
                                <div className='flex items-center'>
                                    ...
                                    <span className='font-demibold'>{chapterIndex+1} {chapter.chapterTitle}</span>
                                </div>
                            ...
                            </div>
                            {
                                !chapter.collapsed && (
                                    <div className='p-4'>
                                        ...
                                        <div className='inline-flex bg-gray-100 p-2 rounded cursor-pointer mb-2' onClick={() => handleLecture('add', chapter.chapterId)}>
                                            + Add lecture
                                        </div>
                                    </div>
                                )
                        ...
                        showPopup && (
                            <div className='fixed inset-0 flex items-center justify-center bg-gray-800 bg-opacity-50'>
                                <div className='bg-white text-gray-700 p-4 rounded relative w-full max-w-80'>
                                    ...
                                    <button onClick={addLecture} type='button' className='rounded w-full bg-blue-400 text-white px-4 py-2'>Add</button>
                                    <img src={assets.cross_icon} alt="" onClick={() => setShowPopup(false)} className='absolute top-6 right-4 w-4 cursor-pointer ' />
                                </div>
